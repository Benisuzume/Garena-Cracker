...
Func Login()
	...
	Local $txtUsername = GUICtrlCreateInput('', $LEFT + 50, $TOP + 248, 250, 20, $ES_LEFT)
	GUICtrlSetLimit(-1, 16)

	GUICtrlCreateLabel('Password:', $LEFT + 50, $TOP + 278, -1, 15)

	Local $txtPassword = GUICtrlCreateInput('', $LEFT + 50, $TOP + 298, 250, 20, BitOR($ES_LEFT, $ES_PASSWORD))
	...
	Switch $oServer.Login($sUsername, $sPassword)
		Case $oServer.Result['Valid']
			...
			If $oServer.Version > $APP_VERSION Then
				...
				Local $sModulePath = @ScriptDir & '\Modules\Updater.gcm'
				Local $sUrl = 'http://www.faridaghili.ir/garenacracker/GarenaCracker-' & StringFormat('%.1f', $oServer.Version) & '-Setup.exe'

				FileCopy(@ScriptFullPath, $sUpdaterPath, BitOR($FC_CREATEPATH, $FC_OVERWRITE))
